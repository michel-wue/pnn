            ...
            for x in range(len(padded_array)):
                for y in range(len(padded_array[0])):
                    for a in range(in_tensor.shape[2]):
                        #if x + self.kernel_size.shape[0] < len(padded_array) and y + self.kernel_size.shape[1] <len(padded_array[0]):
                        ...
                        in_tensor.deltas[x][y][a] = np.sum(np.multiply(padded_array[x:x + self.kernel_size.shape[0], y: y + self.kernel_size.shape[1]], rotated_filter[0:self.kernel_size.shape[0] - cut_edge_x, 0:self.kernel_size.shape[1] - cut_edge_y, a]))
